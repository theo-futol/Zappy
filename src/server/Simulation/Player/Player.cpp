...
Player::Player(int fd, std::shared_ptr<Team> team) : _fd(fd), _pos{0, 0}, rotation(Degrees::NORTH), _team(team), _inventory(), _state(PlayerState::PENDING)
{
    ...
    Logger::log("040", "Player : created", {{"player_id", std::to_string(_fd)}, {"team", _team->_name}, {"x", std::to_string(_pos.x)}, {"y", std::to_string(_pos.y)}});
}
...
void Player::setFrozenUntil(std::chrono::steady_clock::time_point until)
{
    ...
    Logger::log("042", "Player : frozen", {{"player_id", std::to_string(_fd)}, {"until_ms", std::to_string(untilMs)}});
}
