...
std::string Commands::Broadcast(std::vector<std::string> args, Client &client)
{
    ...
    {
        Logger::log("8421", "Broadcast : failed", {{"player_id", std::to_string(client.getFd())}});
        return "ko\n";
    }
    ...
    Logger::log("011", "Broadcast : message sent", {{"player_id", std::to_string(player->getFd())}, {"message", args[0]}});
    return "ok\n";
}
} // namespace zappy
